...
class SiTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0,
                 attn_type="full", na_kernel_size=7, na_dilation=1, attn_dropout=0.0,
                 **block_kwargs):
        super().__init__()
    ...
    def forward(self, x, c):
        ...
        x = x + gate_msa.unsqueeze(1) * self.attn(x_norm, grid_h=grid_size, grid_w=grid_size)
...
class SiT(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            SiTBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio,
                     ...
                     na_dilation=na_dilation, attn_dropout=attn_dropout)
    ...
    def get_attention_weights(self, x, t, y=None):
        """
        Extract attention weights from all layers for measurement.

        Args:
            x: input latent [B, C, H, W] (4 channels, 32x32 typical for 256x256 images)
            t: timestep [B] in [0, 1]
            y: class labels [B] or None (None = unconditional / null token)

        Returns:
            List of attention weight tensors, one per layer.
            Each: [B, heads, N, N] (post-softmax probabilities).
            None entries for blocks that don't support extraction.
        """
        B = x.shape[0]
        N = self.x_embedder.num_patches
        grid_size = int(N ** 0.5)

        tokens = self.x_embedder(x) + self.pos_embed  # [B, N, D]
        ...
        if y is not None:
            ...
            null_y = torch.full((B,), self.y_embedder.num_classes,
                                device=x.device, dtype=torch.long)
            y_emb = self.y_embedder(null_y, train=False)

        c = t_emb + y_emb  # [B, D]

        all_attn = []
        for block in self.blocks:
            # Extract attention before the block processes tokens
            ...
            # Run the block to get input for the next layer
